...
Below I am representing graphically the distribution of grades at the Exam conditioned by the number of missing grades for the online tests. There are two approaches to this question so I will do them both below, beginning with the dplyr row-wise functionality:
```{r}
...
qu15onlinegradesmissingbystudentsboxplot <- gradeswithmissingonlineexam %>% ggplot(aes(x = MissingOnlineGrades, y= Exam)) +
  ...
  theme_minimal()
```
The second approach below is by joining the results with the original table:
```{r}
